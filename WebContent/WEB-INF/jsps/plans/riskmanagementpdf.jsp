...
<body>
	...
	<section class="principal">
		...
		<article id="rightMember" class="contenido">
			<div id="member">
				<h2 id="riskM"><s:text name="pdf.riesgos"/></h2>													 
					<s:set var="idenSize">${applicationScope.iden.size()}</s:set>
					<s:set var="nombrePlan"><s:text name="pdf.plan"/></s:set>
					<s:set var="nombreIden"><s:text name="pdf.iden"/></s:set>									  							
    				<table id="riskM">
    					<tr>
    						...
    						<td>
    							<ul class="navegador">    								    								
    								<s:iterator value="%{#application.info}">     								  									 
				       					<li class="steps" id="PDFRisk"><s:a action="CreatePDF" tabindex="7"><s:text name="pdf.plan"/><s:param name="option" value="1" /><s:param name="title" value="%{#nombrePlan}" /></s:a></li>				       					
				       				</s:iterator>    									    							  									 					 															  																	
								</ul>
							</td>    						
    					</tr>
    						...
    						<td>
    							<ul class="navegador">
    								<s:if test='%{#idenSize > 0}'>
				       					<li class="steps" id="PDFRisk"><s:a action="CreatePDF" tabindex="7"><s:text name="pdf.iden"/><s:param name="option" value="2" /><s:param name="title" value="%{#nombreIden}" /></s:a></li>				       									  
				       				</s:if>				       								       				    									    							  									 					 															  																
								</ul>
							</td>    						
    					</tr>
    					<tr>
    						<th><s:text name="pdf.anali"/></th>
    					</tr>
    					<tr>
    						<td>
    							<ul class="navegador"> 
    								...
    								<s:iterator value="%{#application.study}"> 
    									<s:set var="nombreAnali"><s:text name="pdf.anali"/></s:set>
    								   	<s:set var="nombreAnali">${nombreAnali} ${nombre}</s:set>
				       					<li class="steps" id="PDFRisk"><s:a action="CreatePDF" tabindex="7"><s:text name="pdf.anali"/> <s:property value="nombre"/><s:param name="option" value="3" /><s:param name="num" value="%{#numRisk}" /><s:param name="title" value="%{#nombreAnali}" /></s:a></li>				       					
				       					<s:set var="numRisk">${numRisk+1}</s:set>
				       					<s:set var="nombreAnali"></s:set>
				       				</s:iterator>    									    							  									 					 															  																	
								</ul>
							</td>    						
    					</tr>    					
